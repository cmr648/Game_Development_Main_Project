...
public class Box_Green_Health : MonoBehaviour {
	...
	void FixedUpdate () // an update function that is called every frame
	{
		...
		if (Start_Health <= Half_Health) { // checking to see if the enemy health is less than or equal to 2
			Enemy_Renderer.color = Color.green; // assinging the red color to the enemy
			Right_Top_Eye.color = Color.green; // aassinging a blue color
			Right_Bottom_Eye.color = Color.green; // assinginging a blue color
			Left_Top_Eye.color = Color.green; // assigning a blue color
			Left_Bottom_Eye.color = Color.green; // assiging a blue color
