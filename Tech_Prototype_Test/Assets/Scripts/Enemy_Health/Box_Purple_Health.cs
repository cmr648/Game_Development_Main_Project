...
public class Box_Purple_Health : MonoBehaviour {
	...
	public ParticleSystem Enemy_Death_Splat; // creating an enemy death splat particle reference for a splat for the enmy to leave behind
	...
	void FixedUpdate () // an update function that is called every frame
	{
		...
		if (Start_Health <= Half_Health) { // checking to see if the enemy health is less than or equal to 2
			Enemy_Renderer.color = Color.magenta; // assinging the magenta color to the enemy
			Right_Top_Eye.color = Color.magenta; // aassinging a magenta color
			Right_Bottom_Eye.color = Color.magenta; // assinginging a magenta color
			Left_Top_Eye.color = Color.magenta; // assigning a magenta color
			Left_Bottom_Eye.color = Color.magenta; // assiging a magenta color
